    ...
    return can.Control.extend({
      ...
      tagMap: {
        ...
        "22": {
          type: "h5",
          name: "积分兑换",
          url:"http://app.lehumall.com/html5/BL/coupons.html?from=app"
        },
        "25": {
          type: "native",
          name: "每日签到"
        ...
        "3": {
          type: "h5",
          name: " 火车票",
          url: 'http://m.ctrip.com/webapp/train/v2/index.html?allianceid=30613&sid=929712&popup=close&hiderecommapp=1&page=index#!/index',
        },
        "5": {
          type: "h5",
          name: "日韩馆",
      ...
      renderProductList: function(data) {
        ...
        for (var k = 0; k < prommotionLayout.length; k++) {
          ...
          for (var i = 0; i < prommotionLayout_detail.length; i++) {
            ...
            var DISPRICE = String(prommotionLayout_detail[i]['DISCOUNT_PRICE']);
            var K = Math.floor(DISPRICE);
            var L = (DISPRICE).slice(-2);
            ...
            if(prommotionLayout_detail[i]['DISCOUNT_PRICE']){
              html += "<span>￥" + K+ ".<i>" + L+ "</i><em>" + prommotionLayout_detail[i]['GOODS_PRICE'] + "</em></span>";
            }
            else{
